...
namespace AgMaGest.C_Presentacion.Cajero
{
    public partial class GenerarPagoCEmpresa : Form
    {
        ...
        private void CargarDatosCliente(string cuilCuit)
        {
            ...
            ClienteEmpresa cliente = clienteDAL.ObtenerClienteEmpresaPorCUIT(cuilCuit);

            if (cliente != null)
            {
                TBCuitPago.Text = cliente.CuitCEmpresa;
                TBRazonSocialPago.Text = cliente.RazonSocialCEmpresa;
                TBEmailPago.Text = cliente.EmailCliente;
                TBCelularPago.Text = cliente.CelularCliente;
            }
        }
